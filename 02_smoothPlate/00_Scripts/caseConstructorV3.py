...
dA      = 1.25*0.25e-3                                                        #width of 1 cell
dL      = 1.25*0.25e-3                                                       #length of 1 cell
...
if CHILD:
    parentCase="../iF_Re30.0_60.0_H2OCk_SMV2_INIT"                      #address to the parent case
    baseCase  ='../02_baseCaseMapped/'                                  #folder with baseCase
    caseDir     = ('../iF_Re' + 
                repr(round(Re,4)) + '_' + repr(round(alpha*180/math.pi)) + 
                '_' + liqName + '_SMV2_MAPPED/'
                )
else:
    baseCase  ='../01_baseCaseInit/'
    caseDir     = ('../iF_Re' + 
                repr(round(Re,4)) + '_' + repr(round(alpha*180/math.pi)) + 
                '_' + liqName + '_SMV2_VCell%.3fmm3/'%(dA*dL*dH*1e9)
                )
...
fblockMeshGen(caseDir,hIn,geomSize,cellSize,
    spGrad,mScale)
print 'DONE=======================================\n\n'
