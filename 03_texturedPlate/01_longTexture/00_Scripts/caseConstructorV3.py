...
endTime     = 2.0                                                       #simulation endTime
...
if CHILD:
    parentCase="../iF_Re30.0_60.0_H2OCk_LTV2_INIT"                      #address to the parent case
    baseCase  ='../02_baseCaseMapped/'                                  #folder with baseCase
    caseDir     = ('../iF_Re' + 
                repr(round(Re,4)) + '_' + repr(round(alpha*180/math.pi)) + 
                '_' + liqName + '_LTV2_MAPPED/'
                )
else:
    baseCase  ='../01_baseCaseInit/'
    caseDir     = ('../iF_Re' + 
                repr(round(Re,4)) + '_' + repr(round(alpha*180/math.pi)) + 
                '_' + liqName + '_LTV2_VCell%.3fmm3/'%(dA*dL*dH*1e9)
                )
