...
def fblockMeshGen(caseDir,												#need to specify case directory
	...
	# write edges
	bMD.write('edges \n( \n')
	bMD.write('\tpolyLine 0 3\n\t(\n')
	bMD.write(
				'\t\t('
				 + 
				 '%5.5e %5.5e %5.5e)'%(0.0,aT/2,hT)
				 +
				 '\n'
			 )
	for j in range(1,nTextElems[0]):
		bMD.write(
					'\t\t('
					 + 
					 '%5.5e %5.5e %5.5e)'%(0.0,j*aT,0.0)
					 +
					 '\n'
				 )
		bMD.write(
					'\t\t('
					 + 
					 '%5.5e %5.5e %5.5e)'%(0.0,j*aT+aT/2,hT)
					 +
					 '\n'
				 )
	bMD.write('\t)\n')
	bMD.write('\tpolyLine 1 2\n\t(\n')
	bMD.write(
				'\t\t('
				 + 
				 '%5.5e %5.5e %5.5e)'%(lG,aT/2,hT)
				 +
				 '\n'
			 )
	for j in range(1,nTextElems[0]):
		...
		bMD.write(
					'\t\t('
					 + 
					 '%5.5e %5.5e %5.5e)'%(lG,j*aT+aT/2,hT)
					 +
					 '\n'
				 )
	bMD.write('\t)\n')
